...
final class DashboardRepository extends BaseRepository
{
    ...
    public function metrics(int $tenantId): array
    {
        ...
        $finance = $this->db->prepare(
            'SELECT DATE_FORMAT(due_date, "%Y-%m") AS period, SUM(CASE WHEN type = "income" THEN amount ELSE 0 END) AS income, SUM(CASE WHEN type = "expense" THEN amount ELSE 0 END) AS expense FROM financial_transactions WHERE tenant_id = :tenant_id AND is_active = 1 GROUP BY period ORDER BY period DESC LIMIT 12'
        );
        ...
        $agenda = $this->db->prepare(
            'SELECT id, title, scheduled_at, status FROM work_orders WHERE tenant_id = :tenant_id AND is_active = 1 AND scheduled_at >= NOW() ORDER BY scheduled_at ASC LIMIT 6'
        );
